  ...
  def install_tools(self):
    while True:
      ...
      for tool_name in tool.names:
        print (f" RS[ GR{num} RS] install GR{tool_name}RS")
        num+=1
      print("")
      logo.back()
      cmd=input("RS[GR*RS]GR Choose :RS")
      if cmd=="00" or cmd=="back":
        ...
        try:
          if int(cmd)>=1 and int(cmd)<=int(total):
            os.system("clear")
            logo.installing()
            print("GRinstalling ....RS")
            tool.install(tool.names[int(cmd)-1])
          else:
            print(f"RDSorry '{cmd}' invalid input !!RS")
            sleep(1)
        except ValueError:
  ...
  def category(self):
    while True:
      ...
      for cat in tool.category:
        print (f"  RS[ GR{num} RS] {tool.category_data[cat]}RS")
        num+=1
      print("")
      logo.back()
      cmd=input("RS[GR*RS]GR Choose :RS")
            ...
            while True:
              ...
              for i in tool.names:
                if tool.category[int(cmd)-1] in tool.data[i]["category"]:
                  ...
                  print(f" RS[ GR{cnt} RS] install GR{tool.data[i]['name']}RS")
                  cnt+=1
              print("")
              logo.back()
              tcmd=input("RS[GR*RS]GR Choose :RS")
              if tcmd=="00" or tcmd=="back":
                ...
                try:
                  ...
                  if int(tcmd) in range(1,int(cat_total)+1):
                    ...
                    print(f"RDSorry '{tcmd}' invalid input !!RS")
                    sleep(1)
                except ValueError:
                  print(f"RDSorry '{tcmd}' invalid input !!RS")
                  sleep(1)
